...
# ==============================
    ...
    if ticker is None:
        ...
        st.title("📰 Noticias de Empresas")

        # Selección de empresa y fecha
        ticker = st.selectbox("Seleccione una empresa:", sorted(tickers.keys()))
        year = st.number_input("Año:", min_value=2020, max_value=datetime.date.today().year, value=2023)
        month = st.number_input("Mes:", min_value=1, max_value=12, value=datetime.date.today().month)
        ...
        if st.button("🔍 Buscar noticias"):
            ...
            if not noticias:
                ...
                for n in noticias[:10]:  # máximo 10 por mes o año
                    ...
                    st.markdown(f"**Sentimiento detectado:** {n['sentimiento']}")
                    st.markdown(f"[🔗 Leer más]({n['url']})")
                    st.markdown("---")
