...
if not st.session_state["logged_in"]:
    ...
    if st.session_state["current_page"] == "home":
        ...
        # Imagen desde GitHub RAW (HTML para control de estilo)
        ...
        if st.session_state["current_page"] in page_files:
            page_path = os.path.join(BASE_DIR, "pages", page_files[st.session_state["current_page"]])

            if not os.path.exists(page_path):
                st.error(f"⚠️ No se encontró el archivo en: {page_path}")
            else:
